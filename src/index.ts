...
export function constructBFC(validIds: Set<string>, revokedIds: Set<string>, rHat: number): [typeof BloomFilter[], string] {
...
export function isInBFC(value:string, bfc:typeof BloomFilter[],salted:string): boolean {
   let cascadeLevel=0
...
export function toDataHexString(bfc:[typeof BloomFilter[], string]): string {
  const serializedCascade = bfc[0].map(filter => {
   ...
   const buffer = (filter.buckets instanceof Int32Array) ? filter.buckets.buffer : null;
